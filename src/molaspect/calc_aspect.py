...
def mol_aspect_ratio(got_coords):
    ...
    Returns:
        list: On success::
            ...
            [
                [PC2/PC1, PC3/PC1, PC1 variance],
                [L2/L1, L3/L1, L1],
                [PC1, PC2, PC3],
                [L1, L2, L3],
                [centroid_x, centroid_y, centroid_z],
                [PC1 vector, PC2 vector, PC3 vector],
            ]

        Returns ``[0, 0, 0, 0]`` if ``got_coords`` contains
        ``["error", 0, 0, 0]`` or PCA fails.
    """
    ...
    try:
        ...
        if L1 == 0:
            return [0, 0, 0, 0]
        return [
            [pc2_var / pc1_var, pc3_var / pc1_var, pc1_var],
            [L2 / L1, L3 / L1, L1],
...
def get_aspect_ratio(file_path):
    ...
    Each result is ``[filename, *mol_aspect_ratio(...)]``. After the
    filename, the remaining items match ``mol_aspect_ratio``.
    ...
    Returns:
        list: One row per structure. Returns
        ``[[filename, 0, 0, 0, 0]]`` for an unsupported extension,
    ...
    try:
        ...
        for name, mol in read_mol_file(file_path):
            ...
            results.append([name] + mol_aspect_ratio(got_coords))
        return results
    except ValueError:
